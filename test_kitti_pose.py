...
def main():
    # image_smooth_func = ImageSmoothLayer(3).cuda()
    depth_net = VggDepthEstimator([FLAGS.img_height, FLAGS.img_width])
    depth_net.load_state_dict(torch.load(FLAGS.ckpt_file))
    depth_pyramid_func = ImagePyramidLayer(chan=1, pyramid_layer_num=5).cuda()
    ...
    # for tgt_idx in range(3):
        ...
        frames = Variable(torch.from_numpy(frames_np).cuda().float(), volatile=True)
        ...
        inv_depth_pyramid = depth_net.forward((ref_frame.unsqueeze(0)-127)/127)
        ref_inv_depth_pyramid = [depth.squeeze(0) for depth in inv_depth_pyramid]
        ref_inv_depth0_pyramid = depth_pyramid_func(inv_depth_pyramid[0].squeeze(0))
        ref_frame_pyramid = vo.pyramid_func(ref_frame)
        # smooth image
        # ref_frame_pyramid[0] = image_smooth_func(ref_frame_pyramid[0])
        vo.init(ref_frame_pyramid=ref_frame_pyramid, inv_depth_pyramid=ref_inv_depth_pyramid)
        ...
        out_file = FLAGS.output_dir + '%06d.txt' % (tgt_idx - max_src_offset)
        print(pred_poses)
