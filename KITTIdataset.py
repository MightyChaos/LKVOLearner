...
class KITTIdataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # ref_frame_np = frames[ref_frame_idx, :, :, :]
        # src_frames_np = np.delete(frames, (ref_frame_idx), axis=0)
        # ref_frame = Variable(torch.from_numpy(ref_frame_np).float().cuda())
        # src_frames = Variable(torch.from_numpy(src_frames_np).float().cuda())
        # frames_variable = Variable(torch.from_numpy(frames).float().cuda())
